...
public class CarManager {
    ...
    final public List<Car> findByStandard(final Standard standardType) {
        return vehicles.stream().filter(item -> item.getStandardType() == standardType).collect(Collectors.toList());
    }

    final public List<Car> sortByPrice(final SortOrder sortOrder) {
    ...
    final public List<Car> sortByVolume(final SortOrder sortOrder) {
        List<Car> sortedCars = new ArrayList<>(vehicles);
        if (sortOrder == SortOrder.ASC) {
            sortedCars.sort((vehicle1, vehicle2) ->
